...
import { oversubColor, utilizationColor } from '@/features/metrics/colors';
...
function getDefaultCollapsed(): boolean {
  ...
  // Default: expanded ≥1280px, collapsed below
...
function MetricPill({ label, value, valueClass, icon, title }: MetricPillProps) {
  return (
    <div
            ...
            <div className="flex gap-2 overflow-x-auto">
              ...
              {metrics.choke_point ? (
                <MetricPill
                  ...
                  title={`Choke point: ${metrics.choke_point.fabric_name}`}
                />
              ) : (
                (() => {
              ...
              <MetricPill
                ...
                valueClass={utilizationColor(metrics.power.utilization_pct)}
                icon={<Zap className="size-3.5" />}
                title={`${(metrics.power.total_draw_w / 1000).toFixed(1)} kW / ${(metrics.power.total_capacity_w / 1000).toFixed(1)} kW`}
              />
              ...
              {metrics.port_utilization && metrics.port_utilization.length > 0 && (() => {
                ...
                return (
                  <MetricPill
                    ...
                    icon={<Network className="size-3.5" />}
                    title={`${allocPorts.toLocaleString()} / ${totalPorts.toLocaleString()} ports`}
                  />
                );
              })()}
            </div>
