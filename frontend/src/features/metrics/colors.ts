...
/**
 * Color for port utilization percentages.
 * >80% = red, >60% = amber, else green
 */
...
/**
 * Color for power utilization percentages.
 * >90% = red, >75% = amber, else green
 */
export function powerUtilizationColor(pct: number): string {
  if (pct > 90) return 'text-red-600 dark:text-red-400';
  if (pct > 75) return 'text-amber-600 dark:text-amber-400';
  return 'text-green-600 dark:text-green-400';
}

export function powerUtilizationBgColor(pct: number): string {
